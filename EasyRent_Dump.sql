...
CREATE TABLE maintenance(
	...
	CONSTRAINT fk_bicycle_id FOREIGN KEY (bicycle_id) REFERENCES bicycle(bicycle_id)
);
...
CREATE TABLE replacement(
	...
	CONSTRAINT fk_maintenance FOREIGN KEY (maintenance_id) REFERENCES maintenance(maintenance_id)
);
CREATE TABLE rental(
	rental_id SERIAL,
	account_id INTEGER,
...
CREATE TABLE review(
	...
	CONSTRAINT fk_review FOREIGN KEY (rental_id) REFERENCES rental(rental_id)
);
...
CREATE TABLE rental_association (
	...
	CONSTRAINT fk_bicycle FOREIGN KEY (bicycle_id) REFERENCES bicycle(bicycle_id)
);

-- Insertion dans la table bicycle
INSERT INTO bicycle (bicycle_type, brand, model, lifetime, revision_cycle, last_km_service, status, electric_assistance)
VALUES
  ('VTT', 'Cannondale', 'Trail 7', '2030-12-31', 500, 100, 'Réservé', false),
  ('Route', 'Trek', 'Domane SL', '2032-06-30', 1000, 200, 'Réservé', DEFAULT),
  ('Ville', 'Gazelle', 'Orange C8', '2031-03-15', 750, 50, DEFAULT, false),
  ('Gravel', 'Specialized', 'S-Works', '2033-08-20', 600, DEFAULT, 'Maintenance', true),
  ('BMX', 'Haro', 'Street', '2030-11-05', 450, 80, 'Maintenance', false);

-- Insertion dans la table account
INSERT INTO account (first_name, last_name, hashed_password, email, phone, address, account_role, subscribe)
VALUES
  ('Alice', 'Dupont', 'hashed_pwd1', 'alice.dupont@example.com', '0123456789', '1 Rue de Paris', 'client', DEFAULT),
  ('Bob', 'Martin', 'hashed_pwd2', 'bob.martin@example.com', '0987654321', '2 Avenue de Lyon', 'client', true),
  ('Claire', 'Bernard', 'hashed_pwd3', 'claire.bernard@example.com', '0246813579', '3 Boulevard de Nice', DEFAULT, false),
  ('David', 'Durand', 'hashed_pwd4', 'david.durand@example.com', '0369247518', '4 Place de Bordeaux', 'admin', true),
  ('Emma', 'Lefevre', 'hashed_pwd5', 'emma.lefevre@example.com', '0147258369', '5 Route de Marseille', 'client', false);
...
INSERT INTO rental (account_id, start_date, end_date, payment_status, rental_status)
VALUES
  (1, '2024-07-01', '2024-06-05', 'Payé', 'Terminé'),
  (1, '2024-06-01', '2025-06-05', 'Payé', 'En cours'),
  (2, '2024-06-10', '2025-06-15', 'Non payé', 'En cours'),
  (3, '2024-07-01', '2025-04-05', 'Payé', 'Terminé'),
  (4, '2025-07-10', '2025-07-15', 'Payé', 'Annulé'),
  (5, '2025-04-15', '2025-08-05', 'Non payé', 'Annulé');

-- Insertion dans la table review (rental_id doit référencer rental)
INSERT INTO review (rental_id, rate, description, date_review)
VALUES
  (1, 4.5, 'Très bon service', '2024-06-06'),
